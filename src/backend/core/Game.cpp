...
void Game::updateResult() {
  ...
  // the board is responsible for determining if is a stalemate, 50 move rule
  // and 3 move rule or a checkmate
...
bool Game::submitMove(const std::string &uci) {
  ...
  // Find any move that matches the start and end square, this should be unique,
  // as the start square is unique, it also has to match the promotion tho if
  // the uci is a promotion asking move.
  auto it = std::find_if(legal.begin(), legal.end(), [&](const Move &m) {
    return m.getFrom() == from && m.getTo() == to &&
           (promo == 0 || m.getPromotionPiece() == promo);
  });
