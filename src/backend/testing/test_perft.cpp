...
#include <chrono>
#include <iostream>
#include <vector>   // FIX: Explicitly include vector for the 'moves' error
...
void runPerftWithTiming(int depth, uint64_t expectedNodes) {
    ...
    // FIX: A safer, more explicit way to calculate the duration
    std::chrono::duration<double, std::ratio<1>> time_span = end_time - start_time;
    double seconds = time_span.count();
    
    // Calculate Nodes Per Second (NPS)
    uint64_t nps = (seconds > 0.0) ? static_cast<uint64_t>(actualNodes / seconds) : 0;
